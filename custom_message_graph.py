...
def construct_incremental_continue_graph(df, save_path, features, test=True):
    # If test equals true, construct the initial graph using test_ini_size tweets
    # and increment the graph by test_incr_size tweets each day
    test_ini_size = 500
    test_incr_size = 100
    ...
    for i in range(7, len(distinct_dates) - 1):
        ...
        # All/Relevant Message Strategy: keeping all the messages when constructing the graphs 
        # (for the Relevant Message Strategy, the unrelated messages will be removed from the graph later on).
        G = construct_graph_from_df(incr_df, G)   # 基于前图的基础上增加点和边
        ...
        all_graph_mins.append(grap_mins)
        # extract and save the labels of corresponding tweets
        y = np.concatenate([y, incr_df['event_id'].values], axis = 0)   # 合并之前的label
        np.save(path + 'labels.npy', y)
        ...
        # extract and save the features of corresponding tweets
        indices = incr_df['index'].values.tolist()
        x_incr = features[indices, :]
        x = np.concatenate([x, x_incr], axis = 0)   # 合并之前的feature
        np.save(path + 'features.npy', x)
        print("Features saved.")
        message += "Features saved.\n"
...
save_path = '../incremental_continue_graph_french/'

def create_english_graph():
    ...
    message, data_split, all_graph_mins = construct_incremental_continue_graph(df, save_path, f, True)   #  construct_incremental_dataset_0922
...
def create_french_graph():
    p_part = '../datasets/Twitter/All_French.npy'
    df_np = np.load(p_part, allow_pickle=True)
    print("Data loaded.")
    ...
    # load features
    # the dimension of feature is 300 in this dataset
    f = np.load('../datasets/Twitter/features_69612_0709_spacy_lg_zero_multiclasses_filtered_French.npy')  # 

    # generate test graphs, features, and labels
    message, data_split, all_graph_mins = construct_incremental_continue_graph(df, save_path, f, True)   # construct_incremental_dataset_0922   construct_incremental_continue_graph
    with open(save_path + "node_edge_statistics.txt", "w") as text_file:
        text_file.write(message)
    np.save(save_path + 'data_split.npy', np.asarray(data_split))
    print("Data split: ", data_split)
